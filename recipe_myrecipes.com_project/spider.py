...
class Spider:
    ...
    def gather_links(page_url):
        ...
        try:
            ...
            if page_url.__contains__("www.myrecipes.com/recipe/"):
                with open((Spider.project_name+'/myrecipes_content.txt'),'a',encoding='UTF-8') as f:
                    recipie_url = page_url
                    recipie_title = soup.find("h1",{"class" :"headline heading-content"}).text
                    f.write(recipie_url+"\n")
                    image = False
                    try:
                        recipie_imageurl = soup.find("div",{"class":"image-container"}).find('div',{'class':'component lazy-image lead-media padding-24-bottom medium_2x no-upscale'}).get('data-src')
                        #if recipie_imageurl is None:
                         #   recipie_imageurl = soup.find("video",{"class":"vjs-poster"}).get('poster')
                        image = True
                        print(recipie_imageurl)
                    except:
                        recipie_imageurl = soup.find("div",{"class":"jumpstart-video"}).find('video',{'class':'video-js jumpstart-js'}).get('poster')
                        image = True
                    print(recipie_imageurl)
                    if not image:
                    ...
                    print(recipie_title,recipie_imageurl)
                    try:
                        recipie_content = soup.find("div",{'class':'recipe-summary margin-8-bottom'}).find('div',{'class':'margin-0-auto'}).text
                    except:
                        recipie_content = "None"
                    f.write(recipie_content.strip()+"\n"+"preptime:-\n")
                    try:
                        names = soup.find_all("div",{'class':'recipe-meta-item-header'})
                        prep_counter = False
                        cook_counter = False
                        serves_counter = False
                        for i in range(len(names)):
                            if "active" in str(names[i]).lower() or "prep" in str(names[i]).lower():
                                recipie_prep = soup.find_all("div",{'class':'recipe-meta-item-body'})[i].text
                                prep_counter = True
                                continue
                            if "total" in str(names[i]).lower():
                                recipie_cook = soup.find_all("div",{'class':'recipe-meta-item-body'})[i].text
                                cook_counter = True
                                continue
                            if "yield" in str(names[i]).lower():
                                recipie_serves_list = soup.find_all("div",{'class':'recipe-meta-item-body'})[i].text
                                recipie_serves = "serves "
                                serves_counter = True
                                for serves in recipie_serves_list:
                                    if serves == '(':
                                        break
                                    if serves.isdigit():
                                        recipie_serves += serves
                        ...
                        if not recipie_cook:
                            recipie_cook = "None"
                        if not serves_counter:
                            recipie_serves = 'None'
                    except:
                        recipie_prep = "None"
                        recipie_cook = "None"
                        recipie_serves = 'None'
                    f.write(recipie_prep.strip()+"\n")
                    f.write(recipie_cook.strip()+"\n")
                    f.write(recipie_serves.strip()+"\n")
                    try:
                        ingre_list = soup.find("div",{'class':'ingredients'}).find('ul').find_all('li')
                        ingredients = ""
                        for row in ingre_list:
                            ingredients += (row.text.strip()+"\n")
                    except:
                        ingredients = "None"
                    f.write(ingredients)
                    eol = "*****eol*****"
            ...
            if page_url.__contains__("www.bbc.co.uk/food/recipes/"):
                ...
                with open((Spider.project_name+'/content.txt'),'a',encoding='UTF-8') as f:
                    ...
                    f.write(recipie_title+"\n")
                    try:
                        rating = soup.find("script",{'type':"application/ld+json"})
                        rating = str(rating).strip().split(',')
                        for word in rating:
                            if word.__contains__("aggregateRating"):
                    except:
                        pass
