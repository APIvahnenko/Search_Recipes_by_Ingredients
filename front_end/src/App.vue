...
const testData1 = [
  {
    ...
    id: 1,
    src:
      "https://cube.elemecdn.com/6/94/4d3ea53c084bad6931a56d5158a48jpeg.jpegs",
    title: "title",
    desc: "hi testdata1"
  },
  {
    ...
    async retrieveData() {
      ...
      try {
        const response = await axios.get("http://127.0.0.1:5000/retrieve", {
          params: { recipe_list: this.recipe_idslist, city: "None" }
        });
        //.then((response) => {
        ...
        //修改loading文字
        this.loading = false
        this.loading_text = response.data.recipe_info
        //})
      } catch (error) {
        console.error(error);
      }
