...
<!--        <section>
      ...
      <section class="third col">
        ...
        <el-button v-if = "refresh_button" class="footer-btn" icon="el-icon-arrow-down" circle @click="refreshClick"></el-button>
        <back-to-top></back-to-top>
      </section>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    async mounted() {
      //this.getData();
      this.imgs = [];
	  var time_mess = new Date(); 
	  try{
	  const response= await axios.get('https://www.myperfectrecipe.co.uk/getip',{params: {time:time_mess}}); 
    ...
    methods: {
	  ...
	  try{
	  const response = await axios.get("https://www.myperfectrecipe.co.uk/retrieve", {params: {recipe_list:recipe_idslist,city:this.place,latitude:this.latitude,longitude:this.longitude}});
	  //.then((response) => {
      ...
      async getData() {
		try{
			...
			this.loading = true;
         const response = await axios.get("https://www.myperfectrecipe.co.uk/test", {
              // 传递参数
              params: {
